...
interface AnimationFrameScheme {
  (data: DraggableText[], entropy: number, speed: number, idx: number): AnimeInstance[];
}

enum DivideMode { split = 'split', x = 'x',  y = 'y', xy = 'xy', random = 'random' };
enum SchemeType { happy = 'happy', sad = 'sad',  still = 'still', angry = 'angry', fearful = 'fearful', tender = 'tender', nervous = "nervous" };
...
const getSadSchemeFrame:AnimationFrameScheme = function (data, entropy, speed, idx=0) {
...
const getAngrySchemeFrame:AnimationFrameScheme = function (data, entropy, speed, idx=0) {
  const animationInstances = [] as AnimeInstance[]
  const duration = getDuration(speed);
...
const getFearfulSchemeFrame:AnimationFrameScheme = function (data, entropy, speed, idx=0) {
  const animationInstances = [] as AnimeInstance[]
  const duration = getDuration(speed);
...
const getTenderSchemeFrame:AnimationFrameScheme = function (data, entropy, speed, idx=0) {
  const animationInstances = [] as AnimeInstance[]
  ...
  textElems.forEach((textElem, i) => {
    ...
    const params: AnimeAnimParams = {
      targets: `.${selector}`,
      keyframes: [{ fontVariationSettings: "'wght' 150" ,
          filter: "saturate(60%)"
      },{
...
const getStillSchemeFrame: AnimationFrameScheme = function (data, entropy, duration, idx=0) {
...
class Animator {
  ...
  get splitCenters() {
    ...
    const a = defaultStyleSheet.width / 4, b = defaultStyleSheet.height / 4
  ...
  constructor(data: DraggableText[], entropy: number, speed: number, scheme: string) {
    this.data = data,
    this.speed = speed,
  ...
  splitGroup() {
        ...
        const numWordsPerGroup = Math.max(1, Math.ceil(this.data.length / numGroup));
        ...
        if(this.split!== DivideMode.xy) {
          ...
          for (; (i) * numWordsPerGroup <= assignedWords.length ; i+=1 ) {
            ...
            const endIdx = Math.min(numWordsPerGroup + startIdx, assignedWords.length);
